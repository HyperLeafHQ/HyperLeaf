...
library AssetCatalog {
    ...
    function get(string memory id) internal pure returns (Listing memory a) {
        ...
        if (k == keccak256("hdai") || k == keccak256("hDAI")) {
            return Listing(
                Kind.Liquid,
                "hdai",
                "Hyperleaf Gauntlet DAI",
                "hDAI",
                "gtDAI",
                1,
                30101,
        ...
        if (k == keccak256("hrlusd") || k == keccak256("hRLUSD")) {
            return Listing(
                Kind.Liquid,
                "hrlusd",
                "Hyperleaf senRLUSD",
    ...
    function allIds() internal pure returns (string[25] memory ids) {
        ids = [
            string("hkaito"),
            string("hxsquid"),
            string("hcbeth"),
