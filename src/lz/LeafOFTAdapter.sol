...
contract LeafOFTAdapter is LeafOApp, ReentrancyGuard, LeafYieldFee {
    ...
    /// @dev Recovery by old outbound GUID is intentionally removed until destination
    ///      completion can be proven by an authenticated acknowledgement. An old GUID
    ///      must never authorize consumption of later users' backing.
    function abortCredit(bytes32, address) external pure {
    ...
    function setRateQuote(address q) external onlyOwner {
        if (totalLocked > 0) revert ConfigFrozen();
        _setRateQuote(q);
    }
