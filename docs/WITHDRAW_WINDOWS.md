...
3. HEV `detachmentLockDuration` = **4 days** (HEV on-chain, we do not control it). Live NestVault `0x4f6615…` uses the same 4 days for `dettachForLiquidity`. That is custody, not the hNEST circulation gate (`HNestCirculation`: 8d min delay vs Thursday epoch + 30m).
...
- `attachedAt[tokenId]` — timestamp when vault recorded attach (4d HEV dettach lock).
