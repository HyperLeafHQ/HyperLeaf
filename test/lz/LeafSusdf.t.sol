...
contract MockEndpointSusdf is ILayerZeroEndpointV2 {
...
contract LeafSusdfTest is PegReady {
    MockEndpointSusdf ep;
    ...
    function setUp() public {
        ep = new MockEndpointSusdf();
        inner = new MockConvertERC20("sUSDf", "sUSDf");
        vm.startPrank(owner);
    ...
    function testCatalogPins() public view {
        AssetCatalog.Listing memory a = AssetCatalog.get("hsusdf");
        assertEq(a.innerMainnet, LeafSusdfPolicy.SUSDF);
        assertTrue(a.innerMainnet != LeafSusdfPolicy.USDF);
    ...
    function testRequireSusdfRejectsUsdf() public {
        vm.expectRevert(LeafSusdfPolicy.NotSusdf.selector);
        this._require(LeafSusdfPolicy.USDF);
        LeafSusdfPolicy.requireSusdf(LeafSusdfPolicy.SUSDF);
    }

    function _require(address inner_) external pure {
        LeafSusdfPolicy.requireSusdf(inner_);
    }
