...
contract MockEndpointDai is ILayerZeroEndpointV2 {
...
contract LeafDaiTest is PegReady {
    MockEndpointDai ep;
    ...
    function setUp() public {
        ep = new MockEndpointDai();
        inner = new MockConvertERC20("gtDAI", "gtDAI");
        vm.startPrank(owner);
        adapter = new LeafOFTAdapter(
            address(inner), address(ep), owner, guardian, feeTo, LeafDaiPolicy.DEFAULT_SHARE_CAP
        );
        adapter.setRateKind(LeafYieldFee.RateKind.ConvertToAssets);
        adapter.setRetainRateYield(true);
        adapter.setMaxRateJumpBps(LeafDaiPolicy.MAX_RATE_JUMP_BPS);
        adapter.setConvertYieldToHype(true);
        adapter.setConverter(converter);
        adapter.setHarvester(owner);
        adapter.setPeer(30367, address(1));
        vm.stopPrank();
        _openSrc(adapter, owner, LeafDaiPolicy.DEFAULT_SHARE_CAP);
    ...
    function testCatalogPins() public view {
        ...
        assertEq(a.innerMainnet, LeafDaiPolicy.GTDAI);
        assertTrue(a.innerMainnet != LeafDaiPolicy.DAI);
        assertTrue(a.innerMainnet != LeafDaiPolicy.MORPHO_BLUE);
        assertTrue(a.innerMainnet != LeafDaiPolicy.SMOKEHOUSE_DAI);
        assertEq(a.sourceChainIdMain, 1);
        assertEq(a.sourceEidMain, 30101);
        assertEq(a.defaultCap, LeafDaiPolicy.DEFAULT_SHARE_CAP);
        assertFalse(a.productionEvm);
        assertEq(AssetCatalog.get("hDAI").id, "hdai");
        assertEq(uint256(LeafDaiPolicy.MAX_RATE_JUMP_BPS), 300);
        assertEq(LeafDaiPolicy.CAP_ASSETS, 100_000 ether);
        assertEq(adapter.maxRateJumpBps(), LeafDaiPolicy.MAX_RATE_JUMP_BPS);
    }

    function testRequireGtdaiRejectsDai() public {
        vm.expectRevert(LeafDaiPolicy.NotGtdai.selector);
        this._require(LeafDaiPolicy.DAI);
        vm.expectRevert(LeafDaiPolicy.NotGtdai.selector);
        this._require(LeafDaiPolicy.SMOKEHOUSE_DAI);
        vm.expectRevert(LeafDaiPolicy.NotGtdai.selector);
        this._require(LeafDaiPolicy.MORPHO_BLUE);
        LeafDaiPolicy.requireGtdai(LeafDaiPolicy.GTDAI);
    }

    function _require(address inner_) external pure {
        LeafDaiPolicy.requireGtdai(inner_);
    }
