...
contract LeafListaTest is Test {
    ...
    function testCatalogBatch2() public {
    ...
    function testRateFeedRejectsTokenOrZeroTarget() public {
        vm.startPrank(owner);
        adapter.setRewardsTarget(address(inner));
        vm.expectRevert(LeafYieldFee.BadRateFeed.selector);
        adapter.setRateKind(LeafYieldFee.RateKind.ConvertSnBnbToBnb);
        adapter.setRewardsTarget(address(0));
        vm.expectRevert(LeafYieldFee.BadRateFeed.selector);
        adapter.setRateKind(LeafYieldFee.RateKind.ConvertSnBnbToBnb);
        vm.stopPrank();
    }
    ...
    function testJumpBreakerPinned() public view {
