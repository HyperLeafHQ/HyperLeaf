...
contract MockEndpointRlusd is ILayerZeroEndpointV2 {
...
contract LeafRlusdTest is PegReady {
    MockEndpointRlusd ep;
    ...
    function setUp() public {
        ep = new MockEndpointRlusd();
        inner = new MockConvertERC20("senRLUSDv2", "senRLUSDv2");
        vm.startPrank(owner);
        adapter = new LeafOFTAdapter(
            address(inner), address(ep), owner, guardian, feeTo, LeafRlusdPolicy.DEFAULT_SHARE_CAP
        );
        adapter.setRateKind(LeafYieldFee.RateKind.ConvertToAssets);
        adapter.setRetainRateYield(true);
        adapter.setMaxRateJumpBps(LeafRlusdPolicy.MAX_RATE_JUMP_BPS);
        adapter.setConvertYieldToHype(true);
        adapter.setConverter(converter);
        adapter.setHarvester(owner);
        adapter.setPeer(30367, address(1));
        vm.stopPrank();
        _openSrc(adapter, owner, LeafRlusdPolicy.DEFAULT_SHARE_CAP);
        inner.mint(user, 200 ether);
        vm.deal(user, 1 ether);
    }

    function testCatalogPins() public view {
        AssetCatalog.Listing memory a = AssetCatalog.get("hrlusd");
        assertEq(a.innerMainnet, LeafRlusdPolicy.SEN_RLUSD);
        assertTrue(a.innerMainnet != LeafRlusdPolicy.RLUSD);
        assertTrue(a.innerMainnet != LeafRlusdPolicy.MORPHO_BLUE);
        assertEq(a.sourceChainIdMain, 1);
        assertEq(a.sourceEidMain, 30101);
        assertEq(a.defaultCap, LeafRlusdPolicy.DEFAULT_SHARE_CAP);
        assertFalse(a.productionEvm);
        assertEq(AssetCatalog.get("hRLUSD").id, "hrlusd");
        assertEq(uint256(LeafRlusdPolicy.MAX_RATE_JUMP_BPS), 300);
        assertEq(adapter.maxRateJumpBps(), LeafRlusdPolicy.MAX_RATE_JUMP_BPS);
    }

    function testRequireSenRlusdRejectsRlusd() public {
        vm.expectRevert(LeafRlusdPolicy.NotSenRlusd.selector);
        this._require(LeafRlusdPolicy.RLUSD);
        vm.expectRevert(LeafRlusdPolicy.NotSenRlusd.selector);
        this._require(LeafRlusdPolicy.MORPHO_BLUE);
        LeafRlusdPolicy.requireSenRlusd(LeafRlusdPolicy.SEN_RLUSD);
    }

    function _require(address inner_) external pure {
        LeafRlusdPolicy.requireSenRlusd(inner_);
    }
    ...
    function testVaultMutatorsForbidden() public view {
        assertTrue(LeafForbiddenSelectors.forbidden(bytes4(0x6e553f65)));
        assertTrue(LeafForbiddenSelectors.forbidden(bytes4(0x94bf804d)));
